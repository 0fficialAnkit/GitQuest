...
//                    }
//                }
//                // Handle message array changes (both resets and new messages)
//                .onChange(of: messages.count) { oldCount, newCount in
//                    // If messages were reset (count decreased or changed significantly), reset animation state
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 0) {
            ...
            ScrollViewReader { proxy in
                ...
                // Level changed → reveal all current messages immediately and scroll to top
                .onChange(of: resetId) { _, _ in
                    suppressBottomScroll = true
                    // Pre-populate so every message that already exists is visible instantly.
                    // New messages appended later (not yet in this set) will still fade in via .onAppear.
                ...
                // Handle message array changes (both resets and new messages)
                .onChange(of: messages.count) { oldCount, newCount in
                    // If messages were reset (count decreased or changed significantly), reveal all immediately
                    if oldCount > 0 && (newCount < oldCount || abs(newCount - oldCount) > 2) {
                        animatedMessageIDs = Set(messages.map(\.id))
                        suppressBottomScroll = true
                        Task { @MainActor in
