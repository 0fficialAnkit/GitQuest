...
// MARK: - Repository Snapshot (Value type for per-level storage)

/// A lightweight value-type copy of all repository state fields.
/// Used to save and restore per-level checkpoints.
...
class GitRepositoryState {
    ...
    // MARK: - Shared Instance Reference
    // Allows GameViewModel to access the environment-injected instance
    // without requiring a direct injection point.
    nonisolated(unsafe) static weak var current: GitRepositoryState?
    ...
    init() {
        // Register this instance so GameViewModel can access it
        GitRepositoryState.current = self
    }
    ...
    /// Creates a value-type snapshot of the current repository state.
    ...
    /// Restores the repository to a previously saved snapshot.
